...
const artifactPromptMap: Record<
  PromptId,
  {
    type: ArtifactType;
...
export const artifactService = {
  ...
  getArtifact(projectId: string, artifactId: string): { artifact: Artifact; content: string } {
    const project = projectStore.findById(projectId);
    if (!project) {
      throw new Error('Project not found');
    }
  ...
  async generateArtifactFromPrompt(
    projectId: string,
    input: GenerateDocumentInput,
    generationService: DocumentGenerationService = documentGenerationService,
    ...
    switch (format) {
      case 'json':
        return path.relative(
          projectPaths.projectRoot(projectId),
          path.join(projectPaths.outputJsonDir(projectId), `${artifactId}.json`),
        );
      case 'puml':
        return path.relative(
          projectPaths.projectRoot(projectId),
          path.join(projectPaths.outputPumlDir(projectId), `${artifactId}.puml`),
        );
      case 'md':
        return path.relative(
          projectPaths.projectRoot(projectId),
          path.join(projectPaths.outputMdDir(projectId), `${artifactId}.md`),
        );
      case 'svg':
        return path.relative(
          projectPaths.projectRoot(projectId),
          path.join(projectPaths.outputSvgDir(projectId), `${artifactId}.svg`),
        );
      default:
        return path.relative(
          projectPaths.projectRoot(projectId),
