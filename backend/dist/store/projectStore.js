 ...
 */
const indexStore = new JsonStore(projectPaths.projectIndexFile());
/**
 * Project 本体を扱うストア。
...
export const projectStore = {
    ...
    findAll() {
        ...
        if (!index || !index.projects.length)
            return [];
        return index.projects
            .map((entry) => {
            const store = new JsonStore(projectPaths.projectFile(entry.id));
            return store.read();
        })
     ...
     */
    findById(id) {
        const store = new JsonStore(projectPaths.projectFile(id));
        return store.read();
    },
    /**
     * プロジェクト作成。
    ...
    create(name, description) {
        ...
        new JsonStore(projectPaths.projectFile(id)).write(project);
