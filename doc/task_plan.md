...
| T-06 | 進行中 | `analyzer/` 雛形、CLI、C / C++ / Java 最小パーサ、ingestion agent、unittest を実装中 |
...
今回の着手対象は T-06 のうち、Python 解析基盤の初期化と最小パーサ整備とする。
...
1. `analyzer/` ディレクトリ作成
2. `requirements.txt` と `README.md` を作成
3. 共通パーサインターフェースと `ParseResult` / `Symbol` / `Relationship` を作成
4. C / C++ / Java の最小パーサ雛形を作成
5. `IngestionAgent` と CLI (`parse` / `index`) を作成
6. `unittest` による基礎テストを追加

**次の着手内容**

1. Tree-sitter 導入
2. 現在の正規表現 parser を Tree-sitter 実装へ差し替え
3. 解析結果 JSON の保存フォーマットを backend と接続
4. エビデンス抽出に必要な node / span 情報を追加
