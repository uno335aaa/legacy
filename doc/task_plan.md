...
| データストア | JSON ファイル（ファイルシステム） | RDB 不使用、ORM 不使用 |
...
| タスクID | T-01 |
| 依存 | なし |
| 概要 | 採用する技術スタックを最終確定し、開発環境（Docker, Node.js, Python）を構築する |
...
### T-03: データモデル設計・JSON ストア実装

| 項目 | 内容 |
|------|------|
| タスクID | T-03 |
| 依存 | T-02 |
| 概要 | spec.md §18 に基づくデータモデルを設計し、JSON ファイルベースのストアを実装する |

**実装内容:**

1. 以下のエンティティの JSON スキーマを定義する（spec.md §18 準拠）

   | エンティティ | 保存先 | 主要フィールド |
   |---|---|---|
   | `Project` | `/meta/project.json` | id, name, description, status, created_at, updated_at, active_version_id |
   | `ProjectVersion` | `/meta/versions.json` | id, project_id, label, upload_path, created_at |
   | `AnalysisJob` | `/meta/jobs.json` | id, project_id, version_id, status, started_at, finished_at, current_phase, summary |
   | `Artifact` | `/meta/artifacts.json` | id, project_id, version_id, type, path, format, created_at |
   | `ChatMessage` | `/chat/messages.json` | id, project_id, role, message, related_job_id, created_at |
   | `Evidence` | `/meta/evidences.json` | id, project_id, source_path, symbol_name, evidence_type, summary, confidence |
   | プロジェクト一覧 | `/projects/index.json` | projects[].id, name, status, updated_at |

2. JSON ファイルストアクラスを実装する（読み込み・書き込み・排他制御）
3. 各エンティティの TypeScript 型定義を作成する
4. CRUD ヘルパー関数を実装する
5. サンプルデータ投入スクリプトを用意する

**成果物:**

- [ ] `backend/src/store/jsonStore.ts` — 汎用 JSON ストア
- [ ] `backend/src/store/` — 各エンティティ用ストア
- [ ] TypeScript 型定義ファイル
- [ ] サンプルデータスクリプト
- [ ] データモデル図（Mermaid or PlantUML）

**受入条件:**

- 各エンティティの CRUD 操作が JSON ファイル上で動作する
- ファイルの読み書きが排他制御される
- テストが全て通る
...
2. 中間データ → PlantUML 変換
   - クラス図（§15.3）
3. **PlantUML 構文チェック・再生成ループ**
   - LLM が生成した PlantUML を PlantUML パーサ / コンパイラで構文検証する
   - 構文エラーがある場合、エラー内容を LLM にフィードバックし再生成を指示する
   - 最大 3 回リトライし、それでも解消しない場合はエラーログを残して続行する
4. テンプレートエンジンの導入
...
- [ ] `analyzer/generators/plantuml_validator.py` — 構文チェック・再生成ループ
...
- **LLM 生成の PlantUML が構文検証を通過する（エラー時は最大 3 回再生成される）**
...
4. 変換エラーのハンドリングとログ出力
   - T-13 で構文検証済みの `.puml` を入力とするが、変換時エラーも追加で捕捉する
5. SVG 取得 API
...
- 構文検証済みの `.puml` ファイルから `.svg` ファイルが生成される
- API 経由で SVG を取得できる
- 変換エラー時に適切にログが出力される
...
| T-03 | データモデル設計・JSON ストア実装 | 1 | T-02 | M |
...
gantt
    ...
    T-03 JSONストア設計            :t03, after t02, 2d
...
| LLM 生成の PlantUML に構文エラーが含まれる | SVG 変換失敗、図が欠損 | 構文チェック → エラーフィードバック → 再生成ループ（最大 3 回）を実装 |
...
| JSON ファイルの同時書き込み競合 | データ破損 | ファイルロック機構とアトミック書き込みを実装 |
...
- **LLM 生成の PlantUML は必ず構文検証を行い、エラー時は最大 3 回再生成する**
- **データ永続化は JSON ファイルベースとし、RDB / ORM / マイグレーションは使用しない**
- **MVP では RAG（Retrieval-Augmented Generation）を使用しない**
...
2. **Python ↔ TypeScript 通信方式**: REST API vs サブプロセス呼出し
3. **ジョブキュー**: BullMQ (Redis) vs インプロセスキュー（簡易版で十分か）
4. **Docker 利用有無**: 全コンテナ化 vs ローカル直接実行
5. **テストカバレッジ目標**: 単体テストの必須範囲
